    ...
    I.forEach((u) => {
      if (u.recurrence) {
        ...
        Vn.getDatesFromRRule(u.recurrence[0], u.startTime, this.state.current.minus(G), this.state.current.plus({
          ...
          if (u.cancelledEvents.some((m) => m.isSame(s, "day")))
            return;
          let o;
          const R = u.changedEvents.find((m) => m.originalStartTime.isSame(s, "day"));
          if (R)
            o = {
              name: R.name,
    ...
    return C.forEach((u) => {
      if (u.recurrence) {
        ...
        Vn.getDatesFromRRule(u.recurrence[0], u.startTime, bg(this.state.current), bg(this.state.current).plus({
          ...
          if (u.cancelledEvents.some((m) => m.isSame(s, "day")))
            return;
          const o = u.changedEvents.find((m) => m.originalStartTime.isSame(s, "day"));
          let R;
          if (o)
            R = {
              name: o.name,
