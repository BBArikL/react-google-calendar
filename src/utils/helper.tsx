...
export function isAllDay(startTime: DateTime, endTime: DateTime): boolean {
  return (
    startTime.hasSame(to_datetime(startTime).startOf("day"), "second") &&
...
export function getCalendarURL(
  ...
  if (allDay) {
    url.searchParams.append(
      "dates",
      startTime.toFormat("yyyyMMdd") + "/" + endTime.toFormat("yyyyMMdd"),
    );
  } else {
    url.searchParams.append(
      "dates",
      startTime.toFormat("yyyyMMdd'T'HHmmss") +
        "/" +
        endTime.toFormat("yyyyMMdd'T'HHmmss"),
    );
  }
...
export function isMultiEvent(startTime: DateTime, endTime: DateTime): boolean {
  return (
    endTime.diff(startTime, "hours").hours >= 24 ||
    (!startTime.hasSame(endTime, "day") && endTime.hour >= 12)
  );
}
